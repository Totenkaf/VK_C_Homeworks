...
static void findLocalMax(const int* signals_data, const size_t size, size_t* shared_memory,
                         const size_t size_column, const size_t process_number, const size_t count_processes) {
  ...
  for (size_t i = part_begin; i < part_end; ++i) {
    if (abs(signals_data[i]) > abs(signals_data[i-1]) && abs(signals_data[i]) > abs(signals_data[i+1])) {
  ...
  if (process_number == count_processes - 1 && abs(signals_data[size - 1]) > abs(signals_data[size - 2])) {
...
static size_t countLocalMax(const size_t* shared_memory, const size_t size_column, 
                            const size_t count_processes, const size_t R_window) {
  size_t count = 0;
...
size_t count_R_peaks(const ECG* ecg, const size_t user_cores) {
  ...
  size_t* shared_memory = (size_t *) mmap(NULL, size_column * count_processes * sizeof(size_t),
                                          PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0); // будем делиться памятью между процессами
  ...
  // после завершения дочерних, родительский подсчитает количество локальных экстремум
  size_t count = countLocalMax(shared_memory, size_column, count_processes, ecg->R_window);
